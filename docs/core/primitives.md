...
Tier 1 is the pure function layer. It owns deterministic data transformations that other tiers can reuse.
...
Decode failures raise `DataDecodeError` through the public package surface. The message includes format and position context without echoing raw payloads.
