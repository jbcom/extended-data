...
The old directed inputs package is now first-class under
``extended_data.inputs`` and the package root.
...
Inputs are promoted into Extended Data containers at the boundary, so
decoded JSON/YAML/Base64 payloads can continue into Tier 2 and Tier 3
workflows without a separate adapter layer.
